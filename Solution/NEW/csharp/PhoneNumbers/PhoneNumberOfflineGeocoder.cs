...
namespace PhoneNumbers
{
    ...
    public class PhoneNumberOfflineGeocoder
    {
        ...
        {
            SortedDictionary<int, HashSet<string>> availableDataFiles = new SortedDictionary<int, HashSet<string>>();
            Assembly executingAssembly = Assembly.GetExecutingAssembly();
            string[] manifestResourceNames = executingAssembly.GetManifestResourceNames();
            string prefix = executingAssembly.GetName().Name + "." + this.phonePrefixDataDirectory;
            foreach (string str1 in Enumerable.Where<string>((IEnumerable<string>)manifestResourceNames, (Func<string, bool>)(n => n.StartsWith(prefix))))
            {
                string str2 = str1.Substring(prefix.Length);
                int length = str2.IndexOf("_");
                int key;
                try
                {
                    key = int.Parse(str2.Substring(0, length));
                }
                catch (FormatException)
                {
                    throw new Exception("Failed to parse geocoding file name: " + str2);
                }
                string str3 = str2.Substring(length + 1);
                if (!availableDataFiles.ContainsKey(key))
                    availableDataFiles[key] = new HashSet<string>();
                availableDataFiles[key].Add(str3);
            }
            this.mappingFileProvider = new MappingFileProvider();
            this.mappingFileProvider.ReadFileConfigs(availableDataFiles);
        }
        ...
        {
            Assembly executingAssembly = Assembly.GetExecutingAssembly();
            string name = executingAssembly.GetName().Name + "." + this.phonePrefixDataDirectory + fileName;
            using (Stream manifestResourceStream = executingAssembly.GetManifestResourceStream(name))
            {
                AreaCodeMap areaCodeMap = AreaCodeParser.ParseAreaCodeMap(manifestResourceStream);
