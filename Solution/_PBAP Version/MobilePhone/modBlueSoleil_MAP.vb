...
Module modBlueSoleil_MAP
    ...
    Private Sub BlueSoleil_MAP_InitStruBytes_masLocalServerAttrStru_OLD(ByRef inpByteArray() As Byte, ByVal mapRootPathLocal As String)

        'deprecated this function because the documentation indicates the delimiter should just be a single byte, and not (BTSDK_PBAP_MAX_DELIMITER + 1) bytes
    ...
    Private Sub BlueSoleil_MAP_InitStruBytes_masLocalServerAttrStru(ByRef inpByteArray() As Byte, ByVal mapRootPathLocal As String)
        ...
        Dim sizeOfStru As Integer = 4 + 2 + (BTSDK_PATH_MAXLENGTH + 1) + 1 + 1 + 1
        ReDim inpByteArray(0 To sizeOfStru - 1)
    ...
    Private Sub BlueSoleil_MAP_InitStruBytes_mapGetMessageParam(ByRef inpByteArray() As Byte, ByVal msgHandle As String, ByVal getAttachments As Boolean, ByVal fraction_req As Byte)
        ...
        'fractional req  
        If getAttachments = True Then
            inpByteArray(BTSDK_MAP_MSGHDL_LEN + 1) = fraction_req
        End If
    ...
    Public Function BlueSoleil_MAP_PullMessage(ByVal connHandle As UInt32, ByVal msgHandle As String, ByVal msgFilenameToWriteTo As String, ByVal getAttachments As Boolean) As Boolean
        ...
        BlueSoleil_MAP_InitStruBytes_mapGetMessageParam(struMessageParam, msgHandle, getAttachments, BTSDK_MAP_FRACT_NONE)
        ...
        Dim intHFile_MessageFile As UInt32 = CType(hFile_MessageFile, UInt32)

        Dim retUInt As UInt32
        retUInt = Btsdk_MAPGetMessage(connHandle, struMessageParam(0), intHFile_MessageFile)


        'check fractional resp?
        If struMessageParam(struMessageParam.Length - 1) = BTSDK_MAP_FRACT_RSPMORE Then
            '
    ...
    Public Function BlueSoleil_MAP_PullMessage_MultiPart(ByVal connHandle As UInt32, ByVal msgHandle As String, ByVal msgFilenameToWriteTo As String, ByVal getAttachments As Boolean) As Boolean
        ...
        'get first part.
        BlueSoleil_MAP_InitStruBytes_mapGetMessageParam(struMessageParam, msgHandle, getAttachments, BTSDK_MAP_FRACT_REQFIRST)
        retUInt = Btsdk_MAPGetMessage(connHandle, struMessageParam(0), intHFile_MessageFile)
        ...
        If struMessageParam(struMessageParam.Length - 1) = BTSDK_MAP_FRACT_RSPMORE Then
            'check fractional resp?
            Do
                BlueSoleil_MAP_InitStruBytes_mapGetMessageParam(struMessageParam, msgHandle, getAttachments, BTSDK_MAP_FRACT_REQNEXT)

                retUInt = Btsdk_MAPGetMessage(connHandle, struMessageParam(0), intHFile_MessageFile)

                If struMessageParam(struMessageParam.Length - 1) = BTSDK_MAP_FRACT_RSPLAST Then
                    Exit Do
                End If
