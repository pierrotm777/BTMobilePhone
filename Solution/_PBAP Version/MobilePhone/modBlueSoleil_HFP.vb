...
Module modBlueSoleil_HFP
    ...
    Public Event BlueSoleil_Event_HFP_AudioConnEstablished(ByVal scoConnHandle As UInt16)
    Public Event BlueSoleil_Event_HFP_AudioConnReleased(ByVal scoConnHandle As UInt16)
    ...
    '! UNCLEAR on definition for Btsdk_HFAP_APPRegCbk4ThirdParty.  SDK on website says no return value.  SDK C code shows return UINT32.
    ...
    '<DllImport("BsSDK.dll", CallingConvention:=System.Runtime.InteropServices.CallingConvention.Cdecl)>
    'Private Sub Btsdk_HFAP_APPRegCbk4ThirdParty(ByVal ptrFunc_HFPevent As UInt32)
    'End Sub
    ...
    Private Function BlueSoleil_HFP_Callback_Status_AP_Add(ByVal ptrFunc_HFPEventCallback_AP As IntPtr) As Boolean
        ...
        Btsdk_HFAP_APPRegCbk4ThirdParty(CUInt(ptrFunc_HFPEventCallback_AP))
        Return True
    ...
    Private Sub BlueSoleil_HFP_Callback_Status_AP_Func_ParseExtCmdInd(ByVal ptrExtCmdInd As IntPtr, ByVal paramLen As UInt16)
        ...
        ToLog("AT response is : " & cmdResponse)
        ...
        Dim atMatch As Match = atRegex.Match(cmdResponse.Trim())
        If debugFrm.Visible Then debugFrm.debugTextBox.AppendText(cmdResponse)

        If atMatch.Success Then
            hfpATCmdResult = atMatch.Value 'cmdResponse
            Select Case atMatch.Groups("cmd").Value
                ...
                '    End If
                '    battinfo = ExternalPowerIsON.Split(CChar(","))
                '    'If CDbl(battinfo(0)) = 1 And ExternalPowerIsConnected = False Then
                '    '    ExternalPowerIsConnected = True
                '    '    RaiseEvent BlueSoleil_Event_HFP_ExtPowerBattON()
                '    'ElseIf CDbl(battinfo(0)) = 0 And ExternalPowerIsConnected = True Then
                ...
                Case "CIMI" 'lecture N°ISMI
                    ...
                    '    Dim PhoneDateTime() As String = atMatch.Groups("result").Value.Replace(Chr(34), "").Split(CChar(","))
                    '    PhoneDate = PhoneDateTime(0)
                    '    PhoneTime = Left(PhoneDateTime(1), 8)


                Case "CIND"     'supported indicator types 'accepté par Galaxy A5 (2016)
                    'AT+CIND=?
                    '+CIND: ("CALL",(0,1)),("CALLSETUP",(0-3)),("SERVICE",(0-1)),("SIGNAL",(0-5)),("ROAM",(0,1)),("BATTCHG",(0-5)),("CALLHELD",(0-2))
                    '+CIND: 0,0,1,1,0,3,0
                    Dim PhoneEvent() As String = atMatch.Value.Trim.Split(CChar(":"))
                    Dim CINDbits() As String = PhoneEvent(1).Split(CChar(","))
                    hfpSignalPct = Convert.ToDouble(CINDbits(3))
                    ToLog("AT+CIND return: " & hfpSignalPct.ToString)
                    hfpBatteryPct = Convert.ToDouble(CINDbits(5))
                    ToLog("AT+CIND return: " & hfpBatteryPct.ToString)

            End Select
        End If

        'Not used in my case (replaced by that below)
    ...
    Private Sub BlueSoleil_HFP_Callback_Status_AP_Func_ParsePhoneInfoStru(ByVal ptrStruPhoneInfo As IntPtr, ByVal paramLen As UInt16, ByRef retPhoneNum As String, ByRef retName As String)
        ...
        'ToLog("PhoneInfoStru - NumLen = " & numLen & "   NameLen = " & nameLen)
    ...
    '            Debug.Print("HFP Callback  evt = BTSDK_HFP_EV_AUDIO_CONN_ESTABLISHED_IND")


    '        Case BTSDK_HFP_EV_AUDIO_CONN_RELEASED_IND
    '            'sco conn released.
    '            Debug.Print("HFP Callback  evt = BTSDK_HFP_EV_AUDIO_CONN_RELEASED_IND")
    ...
    Private Sub BlueSoleil_HFP_Callback_Status_AP_Func(ByVal connHandle As UInt32, ByVal hfpEvent As UInt16, ByVal funcParam As IntPtr, ByVal paramLen As UInt16)
        ...
        'ToLog("HFP AP Callback  evt = " & hfpEvent & "  paramLen = " & paramLen)
        ...
        Select Case hfpEvent
            ...
            Case BTSDK_HFP_EV_EXTEND_CMD_IND
                'ToLog("HFP Callback  evt = BTSDK_HFP_EV_EXTEND_CMD_IND")
                If paramLen > 0 Then
            ...
            Case BTSDK_HFP_EV_SIGNAL_STRENGTH_IND
                ToLog("HFP Callback  evt = BTSDK_HFP_EV_SIGNAL_STRENGTH_IND")
                Dim tempSignal As Byte
                tempSignal = Marshal.ReadByte(funcParam)
                If tempSignal < 6 Then
                    BlueSoleil_HFP_LastSignalVal = tempSignal
            ...
            Case BTSDK_HFP_EV_BATTERY_CHARGE_IND
                ToLog("HFP Callback  evt = BTSDK_HFP_EV_BATTERY_CHARGE_IND")
                Dim tempBattery As Byte
                tempBattery = Marshal.ReadByte(funcParam)
                If tempBattery < 6 Then
                    BlueSoleil_HFP_LastBatteryVal = tempBattery
            ...
            Case BTSDK_HFP_EV_SLC_RELEASED_IND
                'HFP connection released?!
                ToLog("HFP Callback  evt = BTSDK_HFP_EV_SLC_RELEASED_IND")
                'RaiseEvent BlueSoleil_Event_HFP_ConnectionReleased()
                Dim t As New Threading.Thread(Sub() RaiseEvent BlueSoleil_Event_HFP_ConnectionReleased())
                t.Start()
            ...
            Case BTSDK_HFP_EV_AUDIO_CONN_ESTABLISHED_IND
                'use this to get conn handle to SCO connection.
                Debug.Print("HFP Callback  evt = BTSDK_HFP_EV_AUDIO_CONN_ESTABLISHED_IND")
                Dim theSCOconnHandle As UInt16
                Dim tempConnHandleINT16 As Int16 = Marshal.ReadInt16(funcParam)
                Dim tempConnHandleBytes(0 To 1) As Byte
                tempConnHandleBytes = BitConverter.GetBytes(tempConnHandleINT16)
                theSCOconnHandle = BitConverter.ToUInt16(tempConnHandleBytes, 0)
                Dim t As New Threading.Thread(Sub() RaiseEvent BlueSoleil_Event_HFP_AudioConnEstablished(theSCOconnHandle))
                t.Start()
            ...
            Case BTSDK_HFP_EV_AUDIO_CONN_RELEASED_IND
                ...
                tempConnHandleBytes = BitConverter.GetBytes(tempConnHandleINT16)
                theSCOconnHandle = BitConverter.ToUInt16(tempConnHandleBytes, 0)
                Dim t As New Threading.Thread(Sub() RaiseEvent BlueSoleil_Event_HFP_AudioConnReleased(theSCOconnHandle))
                t.Start()


            Case BTSDK_HFP_EV_VOICE_RECOGN_ACTIVATED_IND
                ToLog("HFP Callback  evt = BTSDK_HFP_EV_VOICE_RECOGN_ACTIVATED_IND")
                Dim t As New Threading.Thread(Sub() RaiseEvent BlueSoleil_Event_HFP_VoiceCmdActivated())
                t.Start()

            Case BTSDK_HFP_EV_VOICE_RECOGN_DEACTIVATED_IND
                ToLog("HFP Callback  evt = BTSDK_HFP_EV_VOICE_RECOGN_DEACTIVATED_IND")
                Dim t As New Threading.Thread(Sub() RaiseEvent BlueSoleil_Event_HFP_VoiceCmdDeactivated())
                t.Start()
            ...
            Case BTSDK_HFP_EV_ROAMING_ACTIVE_IND
                If hfpIsRoaming = True Then ToLog("HFP Callback  evt = BTSDK_HFP_EV_ROAMING_ACTIVE_IND")
                Dim t As New Threading.Thread(Sub() RaiseEvent BlueSoleil_Event_HFP_StartRoaming())
                t.Start()

            Case BTSDK_HFP_EV_ROAMING_RESET_IND
                If hfpIsRoaming = False Then ToLog("HFP Callback  evt = BTSDK_HFP_EV_ROAMING_RESET_IND")
                Dim t As New Threading.Thread(Sub() RaiseEvent BlueSoleil_Event_HFP_StopRoaming())
                t.Start()
            ...
            Case BTSDK_HFP_EV_CURRENT_CALLS_IND
                'current calls
                ToLog("HFP Callback  evt = BTSDK_HFP_EV_CURRENT_CALLS_IND")
            ...
            Case BTSDK_HFP_EV_CLIP_IND
                'caller id
                'param = Btsdk_HFP_PhoneInfoStru
                'ToLog("HFP Callback  evt = BTSDK_HFP_EV_CLIP_IND")
                Dim clipPhoneNo As String = ""
                Dim clipName As String = ""
                BlueSoleil_HFP_Callback_Status_AP_Func_ParsePhoneInfoStru(funcParam, paramLen, clipPhoneNo, clipName)
                'RaiseEvent Bluesoleil_Event_HFP_CallerID(clipPhoneNo, clipName)
            ...
            Case BTSDK_HFP_EV_SUBSCRIBER_NUMBER_IND
                'local phone no.
                'param = Btsdk_HFP_PhoneInfoStru
                'ToLog("HFP Callback  evt = BTSDK_HFP_EV_SUBSCRIBER_NUMBER_IND")
                Dim subscriberPhoneNo As String = ""
                Dim subscriberName As String = ""
                BlueSoleil_HFP_Callback_Status_AP_Func_ParsePhoneInfoStru(funcParam, paramLen, subscriberPhoneNo, subscriberName)
                'RaiseEvent Bluesoleil_Event_HFP_SubscriberPhoneNo(subscriberPhoneNo, subscriberName)
            ...
            Case BTSDK_HFP_EV_NETWORK_AVAILABLE_IND
                'RaiseEvent BlueSoleil_Event_HFP_NetworkAvailable()
                If hfpNetworkAvailable = False Then 'checked one time only
                    ToLog("HFP Callback  evt = BTSDK_HFP_EV_NETWORK_AVAILABLE_IND")
                    Dim t As New Threading.Thread(Sub() RaiseEvent BlueSoleil_Event_HFP_NetworkAvailable())
                    t.Start()
                End If
            ...
            Case BTSDK_HFP_EV_NETWORK_UNAVAILABLE_IND
                'RaiseEvent BlueSoleil_Event_HFP_NetworkUnavailable()
                If hfpNetworkAvailable = True Then 'checked one time only
                    ToLog("HFP Callback  evt = BTSDK_HFP_EV_NETWORK_UNAVAILABLE_IND")
                    Dim t As New Threading.Thread(Sub() RaiseEvent BlueSoleil_Event_HFP_NetworkUnavailable())
                    t.Start()
                End If

            Case BTSDK_HFP_EV_NETWORK_OPERATOR_IND
                'ToLog("HFP Callback  evt = BTSDK_HFP_EV_NETWORK_OPERATOR_IND")
                Dim netopName As String = ""
                BlueSoleil_HFP_Callback_Status_AP_Func_ParseCOPSinfoStru(funcParam, paramLen, netopName)
                'RaiseEvent BlueSoleil_Event_HFP_NetworkOperatorName(netopName)
                Dim t As New Threading.Thread(Sub() RaiseEvent BlueSoleil_Event_HFP_NetworkOperatorName(netopName))
            ...
            Case BTSDK_HFP_EV_MICVOL_CHANGED_IND
                ToLog("HFP Callback  evt = BTSDK_HFP_EV_MICVOL_CHANGED_IND")
                Dim tempVol As Byte
                Dim tempVolPct As Double
                tempVol = Marshal.ReadByte(funcParam)
                tempVol = tempVol
            ...
            Case BTSDK_HFP_EV_SPKVOL_CHANGED_IND
                ToLog("HFP Callback  evt = BTSDK_HFP_EV_SPKVOL_CHANGED_IND")
                Dim tempVol As Byte
                Dim tempVolPct As Double
                tempVol = Marshal.ReadByte(funcParam)
                tempVol = tempVol
    ...
    Public Function BlueSoleil_HFP_SetMicVol(ByVal connHandle As UInt32, ByVal micVolPct As Double) As Boolean
        ...
        Dim tempVal As Integer = CInt(micVolPct) 'CInt(micVolPct * 15 / 100)
    ...
    Public Function BlueSoleil_HFP_SetSpeakerVol(ByVal connHandle As UInt32, ByVal spkVolPct As Double) As Boolean
        ...
        Dim tempVal As Integer = CInt(spkVolPct * 15 / 100) 'CInt(spkVolPct * 15 / 100)
        Dim micVolByte As Byte = CByte(tempVal)
        If micVolByte > 15 Then micVolByte = 15
        If micVolByte < 0 Then micVolByte = 0
...
#Region "Add Pierrot"
    Public Function ATExecute(ByVal cmd As String, ByRef timeout As UInt32) As Boolean
        Return BlueSoleil_HFP_SendATcmd(hfpHandleConnHFAG, cmd, timeout)
    End Function
    ...
    Public Sub dtmf(ByVal num As String)
        If num.Length = 1 Then
            ...
            Btsdk_HFAP_TxDTMF(hfpHandleConnHFAG, bytes(0))
        End If
    End Sub

    Public Function BlueSoleil_HandsFree_Redial() As Boolean
        Dim retUInt32 As UInt32
        retUInt32 = Btsdk_HFAP_LastNumRedial(hfpHandleConnHFAG)
        If retUInt32 = BTSDK_OK Then
            Return True
        Else
            Return False
