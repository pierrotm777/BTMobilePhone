...
Module modBlueSoleil_PAN
    ...
    'Public BlueSoleil_PAN_IPaddress As String = ""
    ...
    'use this if you want to register a callback to receive info, such as IP address.  I'm not using it at this time.  
    ...
    Private Sub BlueSoleil_PAN_Callback_PANevent(ByVal inpEvent As UInt16, ByVal inpLen As UInt16, ByVal ptrParam As IntPtr)
        ...
        'ToLog("BlueSoleil_PAN_Callback_PANevent  length = " & inpLen)
        ...
        'ToLog("BlueSoleil_PAN_Callback_PANevent  ip = " & IPstr
        ToLog("PAN ip = " & IPstr)

        panIPaddress = IPstr

        'RaiseEvent BlueSoleil_Event_PAN_IPchanged(IPstr)
        Dim t As New Threading.Thread(Sub() RaiseEvent BlueSoleil_Event_PAN_IPchanged(panIPaddress))
        t.Start()
