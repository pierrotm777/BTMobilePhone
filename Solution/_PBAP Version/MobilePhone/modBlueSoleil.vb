...
Module modBlueSoleil
    ...
    <UnmanagedFunctionPointer(CallingConvention.Cdecl)> Public Delegate Function btCallback_PinCode(ByVal dvcHandle As UInt32) As Byte  ' Sub btCallback_PinCode(ByVal dvcHandle As UInt32)
    Public delegateBTpinCodeInd As btCallback_PinCode = AddressOf BlueSoleil_Status_Callback_PinCode
    ...
    Public Event BlueSoleil_DiscoverDevices_DeviceFound(ByVal dvcHandle As UInt32)
    Public Event BlueSoleil_DiscoverDevices_Done()


    Public Event BlueSoleil_Status_PairingPinCode(ByVal dvcHandle As UInt32)
    ...
    Public BlueSoleil_PDU_MTI_DELIVER As Integer = 0
    Public BlueSoleil_PDU_MTI_SUBMIT As Integer = 1




    <DllImport("BsSDK.dll", CallingConvention:=System.Runtime.InteropServices.CallingConvention.Cdecl)>
    Private Function Btsdk_GetRemoteLEDeviceHandle(ByRef inp8bytes_dvcAddr As Byte, ByVal addr_type As Byte) As UInt32  'returns LE Dvc Handle
    End Function
    ...
    <DllImport("BsSDK.dll", CallingConvention:=System.Runtime.InteropServices.CallingConvention.Cdecl)>
    Public Function Btsdk_PinCodeReply(ByVal dvcHandle As UInt32, ByRef arrayPINBytes As Byte, ByVal arrayLen As UInt16) As UInt32
    End Function
    ...
    <DllImport("BsSDK.dll", CallingConvention:=System.Runtime.InteropServices.CallingConvention.Cdecl)>
    Private Function Btsdk_StartEnumConnection() As UInt32         'returns enum handle.
    End Function

    <DllImport("BsSDK.dll", CallingConvention:=System.Runtime.InteropServices.CallingConvention.Cdecl)>
    Private Function Btsdk_EnumConnection(ByVal enumHandle As UInt32, ByRef struConnectionProp As Byte) As UInt32     'returns conn handle.
    End Function

    <DllImport("BsSDK.dll", CallingConvention:=System.Runtime.InteropServices.CallingConvention.Cdecl)>
    Private Function Btsdk_EndEnumConnection(ByVal enumHandle As UInt32) As UInt32
    End Function
    ...
    Private Const BTSDK_CLS_GAP As UInt32 = &H1800
    Private Const BTSDK_CLS_GATT As UInt32 = &H1801
    ...
    Private Const BTSDK_IOCAP_REQ_IND As UInt32 = &HC
    Private Const BTSDK_USR_CFM_REQ_IND As UInt32 = &HD
    Private Const BTSDK_PASSKEY_REQ_IND As UInt32 = &HE
    Private Const BTSDK_REM_OOBDATA_REQ_IND As UInt32 = &HF
    ...
    Private Const BTSDK_VENDOR_EVENT_IND As UInt32 = &HB
    Private Const BTSDK_CONNECTION_COMPLETE_IND As UInt32 = &H8
    ...
    Public Function BlueSoleil_Status_RegisterCallbacks() As Boolean
        ...
        '      funcPtr3 = Marshal.GetFunctionPointerForDelegate(delegateBTinquiryResultInd)
        ...
        'register pairing request pin-code event callback
        Dim funcPtr4 As IntPtr
        funcPtr4 = Marshal.GetFunctionPointerForDelegate(delegateBTpinCodeInd)
        Dim funcPtr4INT As UInt32 = CUInt(funcPtr4)
        BlueSoleil_Stru_CallBack_Init(struCbkBytes, BTSDK_PIN_CODE_IND, funcPtr4INT)
        retUInt32 = Btsdk_RegisterCallback4ThirdParty(struCbkBytes(0)) ' Btsdk_RegisterCallbackEx(struCbkBytes(0), BTSDK_CLIENTCBK_PRIORITY_HIGH)
    ...
    Private Sub BlueSoleil_Stru_ConnectionProperty_GetInfo_FromPtr(ByVal ptrStru_ConnProps As IntPtr, ByRef retRole As Byte, ByRef retResult As UInt32, ByRef retDvcHandle As UInt32, ByRef retSvcHandle As UInt32, ByRef retSvcClass As UInt16, ByRef retDurationSecs As UInt32, ByRef retBytesReceived As UInt32, ByRef retBytesSent As UInt32)
        ...
        BlueSoleil_Stru_ConnectionProperty_GetInfo(evtData, retRole, retResult, retDvcHandle, retSvcHandle, retSvcClass, retDurationSecs, retBytesReceived, retBytesSent)
    ...
    Private Sub BlueSoleil_Stru_ConnectionProperty_GetInfo(ByRef evtData() As Byte, ByRef retRole As Byte, ByRef retResult As UInt32, ByRef retDvcHandle As UInt32, ByRef retSvcHandle As UInt32, ByRef retSvcClass As UInt16, ByRef retDurationSecs As UInt32, ByRef retBytesReceived As UInt32, ByRef retBytesSent As UInt32)
        ...
        Dim struLen As Integer = 26
        If evtData.Length < struLen Then Exit Sub
    ...
    Private Function BlueSoleil_Status_Callback_PinCode(ByVal dvcHandle As UInt32) As Byte

        Debug.Print("BlueSoleil_Status_Callback_PinCode - dvcHandle = " & dvcHandle)

        Dim t As New Threading.Thread(Sub() RaiseEvent BlueSoleil_Status_PairingPinCode(dvcHandle))
        t.Start()


        Return BTSDK_CLIENTCBK_HANDLED   'or BTSDK_CLIENTCBK_NOTHANDLED  ??
    ...
    Private Sub BlueSoleil_Status_Callback_InquiryResult(ByVal dvcHandle As UInt32)
        ...
        ''add dvc handle to array of found devices...  nah.  this is the programmer's responsibility.
        ...
        Dim t As New Threading.Thread(Sub() RaiseEvent BlueSoleil_DiscoverDevices_DeviceFound(dvcHandle))
        t.Start()
    ...
    Private Sub BlueSoleil_Status_Callback_InquiryComplete()
        ...
        Dim t As New Threading.Thread(Sub() RaiseEvent BlueSoleil_DiscoverDevices_Done())
        t.Start()
    ...
    Private Sub BlueSoleil_Status_Callback_ConnEvent(ByVal connHandle As UInt32, ByVal evtType As UInt16, ByVal ptrArgs As IntPtr)
        ...
        If ptrArgs <> IntPtr.Zero Then
            BlueSoleil_Stru_ConnectionProperty_GetInfo_FromPtr(ptrArgs, propRole, propResult, propDvcHandle, propSvcHandle, propSvcClass, propDuration, propBytesReceived, propBytesSent)
        End If
    ...
    Public Function BlueSoleil_GetAllDevices_NamesAndHandles(ByRef retDvcNames() As String, ByRef retDvcHandles() As UInt32, ByRef retDvcCount As Integer, Optional ByVal maxWait_Seconds As UInt16 = 45, Optional ByVal maxDevicesFound As UInt16 = 0, Optional ByVal dvcClass_ToFind As UInt32 = 0) As Boolean
        ...
        retBool = BlueSoleil_GetAllDevices(retDvcHandles, retDvcCount, maxWait_Seconds, maxDevicesFound, dvcClass_ToFind)
    ...
    Public Function BlueSoleil_GetAllDevices(ByRef retDvcHandles() As UInt32, ByRef retDvcCount As Integer, Optional ByVal maxWait_Seconds As UInt16 = 45, Optional ByVal maxDevicesFound As UInt16 = 0, Optional ByVal dvcClass_ToFind As UInt32 = 0) As Boolean
        ...
        Dim hEnum As UInt32 = Btsdk_StartEnumRemoteDevice(0, 0)

        Dim struConnProps(0 To 0) As Byte

        Dim tempDvcHandle As UInt32, tempDvcAddr As String = "", tempDvcName As String = "", tempDvcClass As UInt32
        Dim tempConnHandle As UInt32 = 0

        retDvcCount = 0

        If hEnum <> 0 Then
            Do
                BlueSoleil_Stru_RemoteDeviceProp_Init(struConnProps)

                tempConnHandle = Btsdk_EnumRemoteDevice(hEnum, struConnProps(0))

                If tempConnHandle = BTSDK_INVALID_HANDLE Then
                    Exit Do
                End If

                BlueSoleil_Stru_RemoteDeviceProp_GetInfo(struConnProps, tempDvcHandle, tempDvcAddr, tempDvcName, tempDvcClass, 0, 0, 0, 0)

                ReDim Preserve retDvcHandles(0 To retDvcCount)

                retDvcHandles(retDvcCount) = tempDvcHandle

                retDvcCount = retDvcCount + 1

            Loop

            Btsdk_EndEnumRemoteDevice(hEnum)

        End If

        Return (retDvcCount > 0)
    ...
    Public Function BlueSoleil_DiscoverDevices_Start(ByRef retDvcHandles() As UInt32, ByRef retDvcCount As Integer, Optional ByVal maxWait_Seconds As UInt16 = 45, Optional ByVal maxDevicesFound As UInt16 = 0, Optional ByVal dvcClass_ToFind As UInt32 = 0) As Boolean
        ...
        If maxWait_Seconds > &H30 Then maxWait_Seconds = &H30
    ...
    Public Sub BlueSoleil_DiscoverDevices_Stop()
    ...
    Private Sub BlueSoleil_Stru_RemoteServiceAttribs_GetInfo(ByRef bArray() As Byte, ByRef retSvcClass As UInt16, ByRef retSvcName As String)
        ...
        If retSvcName = "" Then
            Select Case retSvcClass
                Case &H1800
                    retSvcName = "Generic Access Service"

                Case &H1801
                    retSvcName = "Generic Attribute Service"

                Case &H1805
                    retSvcName = "Current Time Service"

                Case &H180F
                    retSvcName = "Battery Service"
    ...
    Private Sub BlueSoleil_Stru_LocalServiceAttribs_Init(ByRef bArray() As Byte)
        ...
        struSize = 2 + 2 + BTSDK_SERVICENAME_MAXLENGTH + 2 + 2 + 4 + 4  ' + 100  'ensure large enough.
    ...
    Public Function BlueSoleil_ConnectService_BySvcHandle(ByVal dvcHandle As UInt32, ByVal svcHandle As UInt32, ByRef retConnHandle As UInt32) As Boolean
        ...
        If retUInt32 = 778 Then
            ...
            BlueSoleil_DisconnectServiceConns_BySvcHandle(svcHandle)
            retUInt32 = Btsdk_Connect(svcHandle, 0, retConnHandle)
    ...
    Public Function BlueSoleil_ConnectService_ByDvcHandle_GATT(ByVal dvcHandle As UInt32, ByRef retConnHandle As UInt32) As Boolean
        Return BlueSoleil_ConnectService_ByDvcHandleAndSvcClass(dvcHandle, BTSDK_CLS_GAP, retConnHandle)
    End Function
    ...
    Public Function BlueSoleil_ConnectService_ByDvcHandle_HSP(ByVal dvcHandle As UInt32, ByRef retConnHandle As UInt32) As Boolean
        Return BlueSoleil_ConnectService_ByDvcHandleAndSvcClass(dvcHandle, BTSDK_CLS_HEADSET_AG, retConnHandle)
    End Function
    ...
    Public Function BlueSoleil_DisconnectAllConnections(Optional ByVal onlyThisDvcHandle As UInt32 = 0) As Integer

        'returns number of service connections disconnected.

        Dim retInt As Integer = 0

        Dim connHandles(0 To 0) As UInt32, connDvcHandles(0 To 0) As UInt32, connSvcHandles(0 To 0) As UInt32, connSvcClasses(0 To 0) As UShort, connCount As Integer = 0

        BlueSoleil_GetLocalDeviceConnections(connHandles, connDvcHandles, connSvcHandles, connSvcClasses, connCount)

        Dim i As Integer
        Dim disTorF As Boolean = False

        For i = 0 To connCount - 1

            disTorF = False

            If onlyThisDvcHandle <> 0 Then
                If connDvcHandles(i) = onlyThisDvcHandle Then
                    disTorF = BlueSoleil_DisconnectServiceConn(connHandles(i))
                End If
            Else
                disTorF = BlueSoleil_DisconnectServiceConn(connHandles(i))
            End If
    ...
    Public Function BlueSoleil_DisconnectAdvancedAudio() As Integer
        ...
        For i = 0 To connCount - 1
            ...
            Select Case connSvcClasses(i)
                Case BTSDK_CLS_AVRCP_TG, BTSDK_CLS_AUDIO_SOURCE, BTSDK_CLS_ADV_AUDIO_DISTRIB ',BTSDK_CLS_HANDSFREE_AG, BTSDK_CLS_HEADSET_AG
                    disTorF = BlueSoleil_DisconnectServiceConn(connHandles(i))
                    If disTorF = True Then
                        retInt = retInt + 1
                    End If
    ...
    Public Function BlueSoleil_DisconnectServiceConns_BySvcClass(ByVal svcClass As UInt16) As Integer
        ...
        For i = 0 To connCount - 1
            ...
            If connSvcClasses(i) = svcClass Then
    ...
    Public Function BlueSoleil_DisconnectServiceConns_BySvcHandle(ByVal svcHandle As UInt32) As Integer
        ...
        For i = 0 To connCount - 1
            ...
            If connSvcHandles(i) = svcHandle Then
    ...
    Public Function BlueSoleil_GetSvcClass_FromProfileName(ByVal svcName As String) As UShort

        Dim svcClass As UShort = 0

        Select Case svcName

            Case "A2DP"
                svcClass = BTSDK_CLS_AUDIO_SINK

            Case "AVRCP"
                svcClass = BTSDK_CLS_AVRCP_TG

            Case "FTP"
                svcClass = BTSDK_CLS_OBEX_FILE_TRANS

            Case "GATT"
                svcClass = BTSDK_CLS_GATT

            Case "HFP"
                svcClass = BTSDK_CLS_HANDSFREE

            Case "HSP"
                svcClass = BTSDK_CLS_HEADSET

            Case "HFP_AG"
                svcClass = BTSDK_CLS_HANDSFREE_AG

            Case "HSP_AG"
                svcClass = BTSDK_CLS_HEADSET_AG


            Case "HID"
                svcClass = BTSDK_CLS_HID

            Case "MAP"
                svcClass = BTSDK_CLS_OBEX_MESSAGEACCESSSERVER

            Case "OPP"
                svcClass = BTSDK_CLS_OBEX_OBJ_PUSH

            Case "PAN"
                svcClass = BTSDK_CLS_PAN_NAP

            Case "PBAP"
                svcClass = BTSDK_CLS_PBAP_PSE

            Case "SPP"
                svcClass = BTSDK_CLS_SERIAL_PORT
    ...
    Public Function BlueSoleil_DisconnectServiceConns_ByName(ByVal svcName As String) As Integer

        Dim svcClass As UShort = BlueSoleil_GetSvcClass_FromProfileName(svcName)

        If svcClass = 0 Then Return -1
        ...
        If svcName = "HFP" Or svcName = "HSP" Then
            '    BlueSoleil_DisconnectServiceConns_ByName(svcName & "_AG")
        End If


        If svcName = "A2DP" Then
            BlueSoleil_DisconnectServiceConns_BySvcClass(BTSDK_CLS_AUDIO_SOURCE)
        End If
    ...
    Public Sub BlueSoleil_Status_UnregisterCallbacks()
        ...
        ReDim struCbkBytes(0 To 0)
        BlueSoleil_Stru_CallBack_Init(struCbkBytes, BTSDK_PIN_CODE_IND, 0)
        retUInt32 = Btsdk_RegisterCallback4ThirdParty(struCbkBytes(0))
    ...
    Public Function BlueSoleil_GetLocalDeviceConnections(ByRef connHandleArray() As UInt32, ByRef connDvcHandleArray() As UInt32, ByRef connSvcHandleArray() As UInt32, ByRef connSvcClassArray() As UShort, ByRef connCount As Integer) As Boolean

        Dim hEnum As UInt32 = Btsdk_StartEnumConnection()

        Dim struConnProps(0 To 0) As Byte

        Dim tempDvcHandle As UInt32, tempSvcHandle As UInt32, tempSvcClass As UInt16, tempName As String = ""
        Dim tempConnHandle As UInt32 = 0
        ...
        If hEnum <> 0 Then
            Do
                BlueSoleil_Stru_ConnectionProperty_Init(struConnProps)

                tempConnHandle = Btsdk_EnumConnection(hEnum, struConnProps(0))

                If tempConnHandle = BTSDK_INVALID_HANDLE Then Exit Do

                BlueSoleil_Stru_ConnectionProperty_GetInfo(struConnProps, 0, 0, tempDvcHandle, tempSvcHandle, tempSvcClass, 0, 0, 0)

                ReDim Preserve connHandleArray(0 To connCount)
                ReDim Preserve connDvcHandleArray(0 To connCount)
                ReDim Preserve connSvcHandleArray(0 To connCount)
                ReDim Preserve connSvcClassArray(0 To connCount)

                connHandleArray(connCount) = tempConnHandle
                connDvcHandleArray(connCount) = tempDvcHandle
                connSvcHandleArray(connCount) = tempSvcHandle
                connSvcClassArray(connCount) = tempSvcClass
    ...
    Public Function BlueSoleil_SvcClass_IsGATT(ByVal svcClass As UInt16) As Boolean
        ...
        For i = 0 To dvcCount - 1
            If dvcNames(i) = profileDvcName Then
                ...
                Select Case profileName
                    ...
                    Case "HSP"
                        connTorF = BlueSoleil_ConnectService_ByDvcHandle_HSP(matchDvcHandle, retConnHandle)
                        retBool = connTorF And (retConnHandle <> 0)
                    ...
                    Case "GATT"
                        connTorF = BlueSoleil_ConnectService_ByDvcHandle_GATT(matchDvcHandle, retConnHandle)
                        retBool = connTorF And (retConnHandle <> 0)
                        retBool = retBool
                ...
                If retBool = False Then
                    ...
                    For j = 0 To svcCount - 1
                        ...
                        Select Case profileName
                            ...
                            Case "GATT"
                                If BlueSoleil_SvcClass_IsGATT(svcClass) = True Then
                                    connTorF = BlueSoleil_ConnectService_BySvcHandle(dvcHandles(i), svcHandles(j), svcConnHandle)
                                    If connTorF = True And svcConnHandle <> 0 Then
                                        retConnHandle = svcConnHandle
                                        retDvcHandle = dvcHandles(i)
    ...
    Public Function BlueSoleil_DevicePairing_PairDevicebyPinCode(ByVal dvcHandle As UInt32, ByVal pinCode As Byte) As Boolean

        Dim retUInt As UInt32
        Dim pinCodeLen As UShort = CUShort(pinCode.ToString.Length)

        retUInt = Btsdk_PinCodeReply(dvcHandle, pinCode, pinCodeLen)
    ...
    Public Function BlueSoleil_DevicePairing_DeleteDevice(ByVal dvcHandle As UInt32) As Boolean
        ...
        'in case it is paired, always unpair before delete.  otherwise, it may end up deleted but still paired?!
        retUInt = Btsdk_UnPairDevice(dvcHandle)
    ...
    Public Function BlueSoleil_GetRemoteDeviceHandle_LE(ByVal dvcAddress As String) As UInt32

        Dim addrBytes(0 To 6) As Byte   '6 + 1

        Dim addrParts(0 To 0) As String
        addrParts = Split(dvcAddress, ":")

        Dim i As Integer
        For i = 5 To 0 Step -1
            addrBytes(5 - i) = CByte(Val("&h" & addrParts(i)))
        Next i

        Dim retInt As UInt32
        retInt = Btsdk_GetRemoteLEDeviceHandle(addrBytes(0), 0)
    ...
    Private Sub BlueSoleil_Stru_RemoteDeviceProp_GetInfo(ByRef bArray() As Byte, ByRef retDvcHandle As UInt32, ByRef retDvcAddr As String, ByRef retDvcName As String, ByRef retDvcClass As UInt32, ByRef lmpFeature As UInt64, ByRef lmpManuCode As UInt16, ByRef lmpSubVersion As UInt16, ByRef lmpVersion As Byte)
        ...
        For i = currByteIdx + 5 To currByteIdx Step -1
            retDvcAddr = retDvcAddr & Strings.Right("0" & Hex(bArray(i)), 2)
        ...
        Dim nullPos As Integer = InStr(1, retDvcName, vbNullChar)
        If nullPos > 0 Then retDvcName = Strings.Left(retDvcName, nullPos - 1)
    ...
    Public Sub BlueSoleil_PDU_TestDecode()

        Dim inpHex As String = "07911326040000F0040B911346610089F60000208062917314080CC8F71D14969741F977FD07"

        inpHex = "0011000B916407861582F50000A704D4E2940A"

        Dim inpHexLen As Integer = Len(inpHex)
        Dim inpBytes(0 To (inpHexLen \ 2) - 1) As Byte
        Dim i As Integer
        Dim j As Integer = 0
        For i = 1 To inpHexLen Step 2
            inpBytes(j) = CByte(Val("&h" & Mid(inpHex, i, 2)))
            j = j + 1
        Next i


        Dim smscNumber As String = "", pduMsgType As Integer, pduMsgTypeDesc As String = ""
        BlueSoleil_PDU_GetMsgType(inpBytes, inpBytes.Length, smscNumber, pduMsgType, pduMsgTypeDesc)

        Dim senderNumber As String = "", protocolId As Byte, EncodingType As Byte, tStamp As DateTime, msgText As String = ""


        Select Case pduMsgType

            Case BlueSoleil_PDU_MTI_DELIVER
                BlueSoleil_PDU_GetInfo_DELIVER(inpBytes, inpBytes.Length, smscNumber, senderNumber, protocolId, EncodingType, tStamp, msgText)

            Case BlueSoleil_PDU_MTI_SUBMIT
                BlueSoleil_PDU_GetInfo_SUBMIT(inpBytes, inpBytes.Length, smscNumber, senderNumber, protocolId, EncodingType, msgText)
    ...
    Public Function BlueSoleil_PDU_GetMsgType(ByRef inpPDUbytes() As Byte, ByVal inpLength As Integer, ByRef retSMSCenterNumber As String, ByRef retMsgTypeID As Integer, ByRef retMsgTypeDesc As String) As Boolean

        retSMSCenterNumber = ""
        retMsgTypeID = -1
        retMsgTypeDesc = "UNKNOWN"
        ...
        Dim inpPDUhex(0 To inpLength - 1) As String

        Dim i As Integer
        For i = 0 To inpLength - 1
            inpPDUhex(i) = Strings.Right("00" & Hex(inpPDUbytes(i)), 2)
        Next i


        Dim inpPDUbin(0 To inpLength - 1) As String
        For i = 0 To inpLength - 1
            inpPDUbin(i) = Strings.Right("00000000" & Convert.ToString(inpPDUbytes(i), 2), 8)
        Next i
        ...
        Dim smscStart As Integer = currIdx
        Dim smscLen As Integer = inpPDUbytes(smscStart) + 1
        Dim smscAddrType As Byte = inpPDUbytes(smscStart + 1)

        Dim tempSMSClen As Integer = smscLen - 2
        For i = smscStart + 2 To smscStart + 2 + tempSMSClen - 1

            If Strings.Mid(inpPDUhex(i), 2, 1) = "F" Then Exit For
            retSMSCenterNumber = retSMSCenterNumber & Strings.Mid(inpPDUhex(i), 2, 1)

            If Strings.Mid(inpPDUhex(i), 1, 1) = "F" Then Exit For
            retSMSCenterNumber = retSMSCenterNumber & Strings.Mid(inpPDUhex(i), 1, 1)

        Next i

        currIdx = currIdx + smscLen

        'skip 1 for First octet of this SMS-DELIVER message
        Dim tempSMSdelCode As Byte = inpPDUbytes(currIdx)
        Dim tempSMSdelCodeBin As String = Strings.Right("00000000" & Convert.ToString(tempSMSdelCode, 2), 8)
        retMsgTypeID = Convert.ToByte(Strings.Right(tempSMSdelCodeBin, 2), 2)


        currIdx = currIdx + 1

        If Strings.Right(tempSMSdelCodeBin, 2) = "00" Then
            retMsgTypeDesc = "SMS-DELIVER"
        ElseIf Strings.Right(tempSMSdelCodeBin, 2) = "01" Then
            retMsgTypeDesc = "SMS-SUBMIT"
        Else
    ...
    Public Function BlueSoleil_PDU_GetInfo_DELIVER(ByRef inpPDUbytes() As Byte, ByVal inpLength As Integer, ByRef retSMSCenterNumber As String, ByRef retOriginatorNumber As String, ByRef retProtocolID As Byte, ByRef retEncodingType As Byte, ByRef retTimeStamp As DateTime, ByRef retMessageText As String) As Boolean

        retSMSCenterNumber = ""
        retOriginatorNumber = ""
        retProtocolID = 0
        retEncodingType = 0
        retTimeStamp = Nothing
        retMessageText = ""
        ...
        Dim tempSMSdelCode As Byte = inpPDUbytes(currIdx)
        Dim tempSMSdelCodeBin As String = Strings.Right("00000000" & Convert.ToString(tempSMSdelCode, 2), 8)
        Dim tempSMSdelCodeDesc As String = "UNKNOWN"
        currIdx = currIdx + 1

        Dim retMsgTypeID As Integer
        retMsgTypeID = Convert.ToByte(Strings.Right(tempSMSdelCodeBin, 2), 2)


        If Strings.Right(tempSMSdelCodeBin, 2) = "00" Then
            tempSMSdelCodeDesc = "SMS-DELIVER"
        ElseIf Strings.Right(tempSMSdelCodeBin, 2) = "01" Then
            tempSMSdelCodeDesc = "SMS-SUBMIT"
        ...
        Dim OriginatorStart As Integer = currIdx
        Dim OriginatorLen As Integer = inpPDUbytes(OriginatorStart)
        Dim OriginatorLenBytes As Integer = OriginatorLen
        If OriginatorLenBytes Mod 2 = 1 Then
            OriginatorLenBytes = OriginatorLenBytes + 1
        End If
        OriginatorLenBytes = OriginatorLenBytes \ 2
        OriginatorLenBytes = OriginatorLenBytes + 1

        Dim OriginatorAddrType As Integer = inpPDUbytes(OriginatorStart + 1)

        currIdx = currIdx + 2
        For i = OriginatorStart + 2 To OriginatorStart + OriginatorLenBytes

            currIdx = currIdx + 1

            If Strings.Mid(inpPDUhex(i), 2, 1) = "F" Then Exit For
            retOriginatorNumber = retOriginatorNumber & Strings.Mid(inpPDUhex(i), 2, 1)

            If Strings.Mid(inpPDUhex(i), 1, 1) = "F" Then Exit For
            retOriginatorNumber = retOriginatorNumber & Strings.Mid(inpPDUhex(i), 1, 1)


        Next i


        'skip 1 for Protocol ID
        Dim tempProtocolID As Byte = inpPDUbytes(currIdx)
        currIdx = currIdx + 1

        'skip 1 for Encoding Scheme
        Dim tempEncodingScheme As Byte = inpPDUbytes(currIdx)
        currIdx = currIdx + 1

        Dim tempEncodingSchemeBin As String = Strings.Right("00000000" & Convert.ToString(tempEncodingScheme, 2), 8)
        Dim tempEncodingSchemeAlphaBits As String = Strings.Mid(tempEncodingSchemeBin, 5, 2)

        'skip 7 for date/time stamp
        Dim dtStampStart As Integer = currIdx
        Dim dtStampYearStr As String = Strings.Mid(inpPDUhex(currIdx + 0), 2, 1) & Strings.Mid(inpPDUhex(currIdx + 0), 1, 1)
        Dim dtStampMonthStr As String = Strings.Mid(inpPDUhex(currIdx + 1), 2, 1) & Strings.Mid(inpPDUhex(currIdx + 1), 1, 1)
        Dim dtStampDayStr As String = Strings.Mid(inpPDUhex(currIdx + 2), 2, 1) & Strings.Mid(inpPDUhex(currIdx + 2), 1, 1)
        Dim dtStampHourStr As String = Strings.Mid(inpPDUhex(currIdx + 3), 2, 1) & Strings.Mid(inpPDUhex(currIdx + 3), 1, 1)
        Dim dtStampMinuteStr As String = Strings.Mid(inpPDUhex(currIdx + 4), 2, 1) & Strings.Mid(inpPDUhex(currIdx + 4), 1, 1)
        Dim dtStampSecondStr As String = Strings.Mid(inpPDUhex(currIdx + 5), 2, 1) & Strings.Mid(inpPDUhex(currIdx + 5), 1, 1)
        Dim dtStampTZoneStr As String = Strings.Mid(inpPDUhex(currIdx + 6), 2, 1) & Strings.Mid(inpPDUhex(currIdx + 6), 1, 1)

        Dim dtDateTime As New DateTime(CInt(dtStampYearStr), CInt(dtStampMonthStr), CInt(dtStampDayStr), CInt(dtStampHourStr), CInt(dtStampMinuteStr), CInt(dtStampSecondStr))

        'begin giant block of crap to deal with timezone offset.
        Dim dtStampTZbyte As Byte = CByte(Val("&h" & dtStampTZoneStr))
        Dim dtStampTZbin As String = Strings.Right("00000000" & Convert.ToString(dtStampTZbyte, 2), 8)
        Dim tzMult As Integer = 1
        If Strings.Mid(dtStampTZbin, 1) = "1" Then tzMult = -1
        Dim tzNumQsPart1 As Integer = Convert.ToInt32(Strings.Mid(dtStampTZbin, 2, 3))
        Dim tzNumQsPart2 As Integer = Convert.ToInt32(Strings.Mid(dtStampTZbin, 5, 4))
        Dim tzNumQs As Integer = (tzNumQsPart1 * 10) + tzNumQsPart2
        Dim tzTemp As Integer = tzNumQs
        If tzTemp Mod 4 <> 0 Then tzTemp = tzTemp - (tzTemp Mod 4)
        Dim tzNumHrs As Integer = tzNumQs \ 4
        Dim tzNumMins As Integer = (tzNumQs Mod 4) * 15

        tzNumHrs = tzNumHrs * tzMult
        tzNumMins = tzNumMins * tzMult
        Dim tzSpan As New TimeSpan(tzNumHrs, tzNumMins, 0)
        Dim dtOffset As New DateTimeOffset(dtDateTime, tzSpan)

        retTimeStamp = dtOffset.LocalDateTime

        currIdx = currIdx + 7



        Dim txtOutLen As Integer = inpPDUbytes(currIdx)
        currIdx = currIdx + 1



        If tempEncodingSchemeAlphaBits = "00" Then
            Dim pdu8bit As String = ""
            Dim pduFloatLen As Integer = 0
            Dim pduFloatStr As String = ""
            Dim pduNewByte As Byte = 0

            For i = currIdx To inpLength - 1

                pdu8bit = pdu8bit & "-0" & Strings.Right(inpPDUbin(i), 7 - pduFloatLen) & pduFloatStr

                pduFloatLen = pduFloatLen + 1
                pduFloatStr = Strings.Left(inpPDUbin(i), pduFloatLen)

                If pduFloatLen = 7 Then
                    pdu8bit = pdu8bit & "-0" & pduFloatStr
                    pduFloatLen = 0
                    pduFloatStr = ""
                End If
            ...
            pdu8bit = Replace(pdu8bit, "-", "")
            Dim tempChar As Byte = 0
            For i = 1 To pdu8bit.Length Step 8
                tempChar = Convert.ToByte(Strings.Mid(pdu8bit, i, 8), 2)

                retMessageText = retMessageText & Chr(tempChar)
            Next i
        ...
        If tempEncodingSchemeAlphaBits = "01" Then
            For i = currIdx To inpLength - 1

                retMessageText = retMessageText & Chr(inpPDUbytes(i))
            Next i
        ...
        If tempEncodingSchemeAlphaBits = "10" Then

            retMessageText = System.Text.Encoding.Unicode.GetString(inpPDUbytes, i, inpLength - i + 1)
    ...
    Public Function BlueSoleil_PDU_GetInfo_SUBMIT(ByRef inpPDUbytes() As Byte, ByVal inpLength As Integer, ByRef retSMSCenterNumber As String, ByRef retDestinationNumber As String, ByRef retProtocolID As Byte, ByRef retEncodingType As Byte, ByRef retMessageText As String) As Boolean

        retSMSCenterNumber = ""
        retDestinationNumber = ""
        retProtocolID = 0
        retEncodingType = 0
        retMessageText = ""
        ...
        Dim tempVPFbin As String = Strings.Right("00000000" & Convert.ToString(tempSMSdelCode, 2), 8)
        tempVPFbin = Strings.Mid(tempVPFbin, 4, 2)
        ...
        'skip 1 for First octet of this SMS-DELIVER message
        Dim retSMSframeNum As Byte = inpPDUbytes(currIdx)
        currIdx = currIdx + 1


        Dim DestinationStart As Integer = currIdx
        Dim DestinationLen As Integer = inpPDUbytes(DestinationStart)
        Dim DestinationLenBytes As Integer = DestinationLen
        If DestinationLenBytes Mod 2 = 1 Then
            DestinationLenBytes = DestinationLenBytes + 1
        End If
        DestinationLenBytes = DestinationLenBytes \ 2
        DestinationLenBytes = DestinationLenBytes + 1

        Dim DestinationAddrType As Integer = inpPDUbytes(DestinationStart + 1)

        currIdx = currIdx + 2
        For i = DestinationStart + 2 To DestinationStart + DestinationLenBytes

            currIdx = currIdx + 1

            If Strings.Mid(inpPDUhex(i), 2, 1) = "F" Then Exit For
            retDestinationNumber = retDestinationNumber & Strings.Mid(inpPDUhex(i), 2, 1)

            If Strings.Mid(inpPDUhex(i), 1, 1) = "F" Then Exit For
            retDestinationNumber = retDestinationNumber & Strings.Mid(inpPDUhex(i), 1, 1)
