...
namespace OpenNETCF
{
	...
	public class MathEx
	{
		...
		{
			return (Math.Exp(angle) - Math.Exp(-angle)) / 
		...
		public class FFTReal
		{
			...
			{
				if(! MathEx.IsPowerOf2(dataLength))
				{
			...
			private unsafe void RunFFT(double[] x, double *sf, double*df)
			{
				...
				fixed(long *bit_rev_lut_ptr = m_brlookup.m_data)
				{
					...
					for (pass = 3; pass < m_bitcount; ++pass)
					{
						...
						fixed(double *cos_ptr = &m_tlookup.m_data[pass])
						{
							do
							{
								long				i;
			...
			public Complex[] Compute(double[] input)
			{
				...
				{
					complex[c].real = output[c * 2];
					complex[c].imaginary = output[(c * 2) + 1];
				}
