...
#Region "SDK Function DLL Import"
    ...
    'op_id can be one of these commands
    ...
    'state_flag can be one of thes commands
    ...
    End Function
    '** Section 6.3.7.2.1 in SDK 2.1.3 **
    <DllImport("BsSDK.dll", CallingConvention:=System.Runtime.InteropServices.CallingConvention.Cdecl)> _
    Private Shared Function Btsdk_RegisterA2DPSNKService(len As UInt16, audio_card As Byte) As UInt32
    End Function
    '** Section 6.3.7.2.2 in SDK 2.1.3 **
    <DllImport("BsSDK.dll", CallingConvention:=System.Runtime.InteropServices.CallingConvention.Cdecl)> _
    Private Shared Function Btsdk_UnregisterA2DPSNKService() As UInt32
    End Function
    ...
    ''** Section 6.3.9.1.23 in SDK 2.1.3 **
    '<DllImport("BsSDK.dll", CallingConvention:=CallingConvention.Cdecl)> _
    'Private Shared Function Btsdk_UnregisterPBAPService(ByVal svc_hdl As UInt32) As UInt32
    'End Function
...
#Region "Variables"
    ...
    'Private pbapEventGlobal As Btsdk_PBAP_STATUS_INFO_CB 'PbabEventCallback
...
#End Region
    ...
    Private Sub startup()
        ...
        If TempPluginSettings.PhoneBookList = "" AndAlso TempPluginSettings.PhoneBookListUpdate = True Then
        ...
        ReadPhoneBooksByATCmds() ' replaced by GetVCF() command
    ...
    Private Sub ReadPhoneBooksByATCmds() 'replaced by GetVCF()
        RaiseEvent SetIndBlinkOn()

        If File.Exists(MainPath & "PhoneBook\MobilePhone_MT.txt") = True Then
            File.Delete(MainPath & "PhoneBook\MobilePhone_MT.txt")
        End If
        Dim QuickBookList = "DC,RC,MC"
        Dim QuickBooKListArray() As String = QuickBookList.Split(",")
        For Each QuickBook As String In QuickBooKListArray 'Read each phone book usable and sav it
            CurrentPhoneBook = QuickBook
            If debugFrm.Visible Then debugFrm.debugTextBox.AppendText("Extracting " & CurrentPhoneBook & " phonebook ..." & vbCrLf & vbCrLf)
            File.Delete(MainPath & "PhoneBook\MobilePhone_" & CurrentPhoneBook & ".txt")
            PhoneBookSyncInProgress = True
            If debugFrm.Visible Then debugFrm.debugTextBox.AppendText("Setting Sync Status ..." & vbCrLf & vbCrLf)
            getPhoneBook(CurrentPhoneBook)
        ...
        If TempPluginSettings.PhoneBookUpdate = True Then 'mise à jour des phone book
            ...
            For Each PhoneBookList As String In PhoneBooKListArray 'Read each phone book usable and sav it
                ...
                If debugFrm.Visible Then debugFrm.debugTextBox.AppendText("Extracting " & CurrentPhoneBook & " phonebook ..." & vbCrLf & vbCrLf)
                If PhoneBookList <> "MT" And PhoneBookList <> "DC" And PhoneBookList <> "RC" And PhoneBookList <> "MC" Then
...
#End Region
    ...
    Private Function HFPInit() As Boolean
        If Btsdk_HFAP_APPRegCbk4ThirdParty(hfEventGlobal) = BTSDK_OK Then
            ...
            '                            BTSDK_AG_BRSF_BVRA Or BTSDK_AG_BRSF_BINP Or BTSDK_AG_BRSF_REJECT_CALL Or _
            '                            BTSDK_AG_BRSF_ENHANCED_CALLSTATUS Or BTSDK_AG_BRSF_ENHANCED_CALLCONTROL Or _
            '                            BTSDK_AG_BRSF_EXTENDED_ERRORRESULT Or BTSDK_CLS_PBAP_PSE)
    ...
    'Private Sub AddContact(nameContact As String)
    '    'Replaced by GetVCF()
    '    'Case "CPBR"
    '    'Number of Records on Phone
    '    'Time to retrieve them all
    ...
    '    'Add to Phonebook
    '    'Dim c As contact = New contact(UInt32.Parse(resultMatch.Groups(1).Value), resultMatch.Groups(4).Value, resultMatch.Groups(2).Value)
    '    Dim c As contact = New contact(UInt32.Parse(resultMatch.Groups(1).Value), resultMatch.Groups(4).Value, "name")
    '    phoneBookData.add(c)
    ...
    '            Exit Sub
    '            'Else
    '            '    MsgBox("PBAP connection selected!")
    '        End If

    '        'Dim regFileIO As UInt32 = Btsdk_PBAPRegisterFileIORoutines(hconn, ptr)
    '        Btsdk_PBAPRegisterFileIORoutines(hconn, ptr)
    '        'Btsdk_PBAPRegisterStatusCallback(hconn, PbapAppStatusCB())

    '        'ajout 11-12-2015
    '        'param.mask = BTSDK_PBAP_PM_LISTOFFSET Or BTSDK_PBAP_PM_MAXCOUNT Or BTSDK_PBAP_PM_ORDER
    '        'param.list_offset = 0
    '        'param.max_count = 65535 '0xFFFF
    ...
    '                Btsdk_PBAPFilterComposer(param.filter, BTSDK_PBAP_FILTER_PHOTO)  '!
    '                Btsdk_PBAPFilterComposer(param.filter, BTSDK_PBAP_FILTER_LOGO)
    ...
    '            MessageBox.Show("The phone book retrieved is saved ")
    '        Else
    '            MessageBox.Show("The phone book retrieved isn't saved ")
    '        End If

    '    Catch ex As Exception
    '        MessageBox.Show(ex.Message, "getPBAPBook function error")
    '        Exit Sub
    '    End Try
    ...
    '            MessageBox.Show("%10d byte / %d transferred", s_cur_size & s_total_size)
    '        Else
    '            s_cur_size += cursize
    '            MessageBox.Show(s_cur_size)
    '        End If
    '    Else
    '        s_cur_size += cursize
    '        MessageBox.Show("\r%10d byte / %d transferred\n", s_cur_size & s_total_size)
    '        s_cur_size = 0
    '        s_total_size = 0
...
#Region "Read Write SMS in PDU mode"
    ''' <summary>
    ''' Initiate the SMS read in PDU mode
    ''' </summary>
    ''' <remarks></remarks>
...
#Region "PDU SMS mode 'Read'"
    ''' <summary>
    ''' Convert a PDU stream in a text stream
    ''' </summary>
    ''' <param name="txtPDU"></param>
    ''' <returns>return txtResult</returns>
    ''' <remarks></remarks>
...
#Region "PDU SMS mode 'Write'"
    ...
    ''' <summary>
    ''' Convert a text in a PDU stream
    ''' </summary>
    ''' <param name="txtServiceCenterNum"></param>
    ''' <param name="txtDestNum"></param>
    ''' <param name="txtUserData"></param>
    ''' <returns>return txtPDU</returns>
    ''' <remarks></remarks>
...
#End Region
    ...
    Private Sub HFPDone()
        ...
        'Btsdk_Disconnect(pbapHandle)
        'Btsdk_UnregisterPBAPService(pbapService)
...
#Region "PBAP by AT command"
    ...
    ''' <summary>
    ''' Update the phone book by extract the vcard from phone
    ''' </summary>
    ''' <remarks></remarks>
    Public Sub MobilePhoneSyncProcess() 'A MODIFIER POUR PB ICH OCH MCH et CCH
        ...
        'workerThread.Abort()
    End Sub
    'Public Sub RRListSyncProcess() 'A MODIFIER POUR PB ICH OCH MCH et CCH
    '    If Not SynchroningInProgress Then
    '        SynchroningInProgress = True
    '        Threading.Thread.Sleep(1000) 'wait some time before engaging this process
    '        Dim PhoneBooKListArray() As String = TempPluginSettings.PhoneBookList.Split(",")
    ...
    ''' <summary>
    ''' Sort a RR Custom List by name (ICO line is accepted)
    ''' </summary>
    ''' <remarks></remarks>
    Public Sub PhoneBookSort()
        ...
        Try
            Dim spLine() As String
            Dim allLines As String() = IO.File.ReadAllLines(MainPath & "PhoneBook\MobilePhone_MT.txt", Encoding.Default)
            PhoneBookArray.Clear()
            ...
            If File.Exists(MainPath & "PhoneBook\MobilePhone_MT2.txt") = True Then
                File.Delete(MainPath & "PhoneBook\MobilePhone_MT2.txt")
            End If

            Dim objStreamWriter As StreamWriter
            objStreamWriter = New StreamWriter(MainPath & "PhoneBook\MobilePhone_MT2.txt", True, Encoding.Default)
            objStreamWriter.WriteLine("0")
            For Each ligne As String In PhoneBookArray
                'MessageBox.Show(ligne)
                spLine = ligne.Split(",")
                objStreamWriter.Write(spLine(1))
                objStreamWriter.Write("||")
                objStreamWriter.WriteLine(spLine(0))
                objStreamWriter.WriteLine(spLine(2))
            Next
            objStreamWriter.Close()
            ...
            'File.Delete(MainPath & "PhoneBook\MobilePhone_" & PhoneListType & ".txt")
            File.Replace(MainPath & "PhoneBook\MobilePhone_MT2.txt", MainPath & "PhoneBook\MobilePhone_MT.txt", Nothing)
        Catch ex As Exception
            MsgBox("Sub PhoneBook Sort - " & ex.Message)
        End Try
        PhoneBookSortInProgress = False
    ...
    Public Sub PhoneBookMerge(ByVal PhoneBookList_A As String, ByVal PhoneBookList_B As String)
        Try
            ...
            For Each line As String In PhoneBookArray_B
                If debugFrm.Visible Then debugFrm.debugTextBox.AppendText("Array B after sort - " & line & vbCrLf)
            Next
...
#Region "Contacts RR List"
    Public Sub AddCustomListDevicesServices(ByVal CustomList As String, ByVal Number As String, ByVal Name As String, ByVal IconType As String)
        ...
        File.AppendAllText(CustomList, "ICO" & IconType & vbCrLf, Encoding.Unicode)
...
#Region "A2DP"
    Public Sub RunA2DP()
        ...
        retUInt32 = Btsdk_RegisterA2DPSNKService(1000, Nothing)
        If retUInt32 = BTSDK_OK Then
            Try
                MessageBox.Show("A2DP is ON", "A2DP error")
            Catch ex As Exception
                MessageBox.Show(ex.Message, "A2DP error")
            End Try
