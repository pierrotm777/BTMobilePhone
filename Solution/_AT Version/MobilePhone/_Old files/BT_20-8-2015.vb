...
#Region "Structures"
    ...
    Public Delegate Function Btsdk_WriteFile_Func(file_hdl As UInt32, buf As Byte, bytes_to_write As UInt32) As Int32
...
#Region "PBAP Constantes"
    ...
    Const BTSDK_PBAP_FILTER_X_IRMC_CALL_DATETIME As UInt32 = &H1C
...
#End Region
    ...
    'Public Delegate Sub pbapEventCallback(ByVal hdl As UInt32, ByVal cb_funcs As BtSdkPBAPSvrCBStru)
    'Public Delegate Sub mapEventCallback(ByVal hdl As UInt32, ByVal reason As UInt16, ByVal arg As IntPtr) 'Event for MAP

    'Public Delegate Sub connEventSendReceivedBytes(ByVal hdl As UInt32, ByVal reason As UInt16, ByVal arg As IntPtr)
...
#Region "SDK Function DLL Import"
    ...
    <DllImport("BsSDK.dll", CallingConvention:=CallingConvention.Cdecl)> _
    Private Shared Function Btsdk_PBAPRegisterSvrCallback(ByVal conn_hdl As UInt32, ByVal cb_funcs As BtSdkPBAPSvrCBStru) As UInt32
    End Function
    '** Section 6.3.9.1.23 in SDK 2.1.3 **
    <DllImport("BsSDK.dll", CallingConvention:=CallingConvention.Cdecl)> _
    Private Shared Function Btsdk_PBAPRegisterFileIORoutines(ByVal svc_hdl As UInt32, ByVal cb_funcs As BtSdkPBAPFileIORoutinesStru) As UInt32
    End Function
    ...
    <DllImport("BsSDK.dll", CallingConvention:=System.Runtime.InteropServices.CallingConvention.Cdecl)> _
    Private Shared Function Btsdk_PBAPPullPhoneBook(ByVal connHandle As UInt32, ByVal path As String, param As BtSdkPBAPParamStru, ByVal file_hdl As UInt32) As UInt32
    End Function
...
#Region "Variables"
    ...
    'Private pbapEvenGlobal As pbapEventCallback
...
#End Region
    ...
    Public Sub contactRetrieve(ByVal contactsNum As UInt32)
        ...
        For PhoneNumberOfContactsLoop = 1 To PhoneNumberOfContactsFound
        ...
        If atMatch.Success Then
            Select Case atMatch.Groups("cmd").Value
                ...
                Case "CMGW" 'This command is used to store message in the SIM
                    ...
                    'MsgBox(atMatch.Value)
                    'If debugFrm.Visible Then debugFrm.debugTextBox.AppendText(result)
                    'Case "CMTI" 'check message (incompatible with my phone)
                    '    Dim sms() As String
                    '    SmsIsReceived = Replace(atMatch.Value, Chr(0), "")
                    '    If Left(SmsIsReceived, 5) = "+CMTI" Then
                    '        SmsIsReceived = Mid(SmsIsReceived, 7).Replace(Chr(34), "").Trim
                    '    End If
                    '    sms = SmsIsReceived.Split(",")
                    '    SmsIsReceivedMemory = sms(0)
                    '    SmsIsReceivedNumber = sms(1)
                    '    SmsIsReceivedStatus = True
                    '    MsgBox(atMatch.Value)
                ...
                Case "CMGR" 'lecture du message sms reçu
                    ...
                    'MessageBox.Show(atMatch.Groups("result").Value.Trim())
                    'MessageBox.Show(MsgBox(atMatch.Value))
                    'SmsToRead = atMatch.Groups("result").Value.Trim()

                Case "CMGL" 'lecture type de sms en  mode pdu
    ...
    'Private Function setpbapCallBacks() As Boolean
    '    Dim cb As BtSdkPBAPSvrCBStru = New BtSdkPBAPSvrCBStru()
    '    cb.func = connEventGlobal
    '    cb.type = BTSDK_CONNECTION_EVENT_IND

    '    Dim ptr As IntPtr = Marshal.AllocHGlobal(Marshal.SizeOf(GetType(BtSdkPBAPSvrCBStru)))
    '    Marshal.StructureToPtr(cb, ptr, True)
    ...
    Public Sub New()
        Try
            name = "MR2"
            initVars()
            'hfagService = BTSDK_INVALID_HANDLE
            'hsagService = BTSDK_INVALID_HANDLE
            hfService = BTSDK_INVALID_HANDLE
            'hsService = BTSDK_INVALID_HANDLE
            ...
            'PBAP event (contacts)
            'pbapEvenGlobal = AddressOf pbapEvent
    ...
    'TEST PBAP
    Private Function connectAG_PABP() As Boolean
        Dim svc_attr As BtSdkRemoteServiceAttrStru
        ...
        For i As Byte = 0 To Marshal.ReadInt32(psvc_num) - 1
            If Btsdk_GetRemoteServiceAttributes(Marshal.ReadInt32(psvc_hndls, i * Marshal.SizeOf(GetType(UInt32))), psvc_attr) = BTSDK_OK Then
                svc_attr = Marshal.PtrToStructure(psvc_attr, GetType(BtSdkRemoteServiceAttrStru))
                'PBAP PSE ----> Fonctionne !!!
                'MessageBox.Show(Hex(svc_attr.service_class))
                If svc_attr.service_class = BTSDK_CLS_PBAP_PSE Then 'BTSDK_CLS_PHONEBOOK_ACCESS Then
                    If Btsdk_ConnectEx(connHandle, BTSDK_CLS_PBAP_PSE, 0, pconn_hndl) = BTSDK_OK Then
                        'MessageBox.Show(Marshal.ReadInt32(psvc_hndls, i * Marshal.SizeOf(GetType(UInt32))))
                        pbapHandle = Marshal.ReadInt32(pconn_hndl)
                    End If
                    RaiseEvent PBAPIsReady()
                    Exit For
                End If
    ...
    Private Sub waitForPhone()
        ...
        While True
            ...
            ' Else
            connectAG_HANDSFREE()
            'connectAG_PABP() 'ajout PBAP 4-8-2015
            'End If
            While True
...
#Region "Test Read Write SMS"
    ...
    Public Function SendSMS(numberphone As String, message As String) As Boolean
        If PDUTextModesUsable = 0 Then 'PDU MODE
            ...
            ATExecute("AT+CMGW=" & CStr(CInt(SmsIsReceivedMemory) + 1) & vbCrLf, 2000)
        ElseIf PDUTextModesUsable = 1 Or PDUTextModesUsable = 2 Then 'TEXT MODE
...
#Region "PDU SMS mode 'Write'"
    ...
    Public Function WritePDUMessage(txtServiceCenterNum As String, txtDestNum As String, txtUserData As String) As String
        ...
        If txtServiceCenterNum.Length = 0 Then MsgBox("Please Enter Service Center Number") : Exit Function
        If txtDestNum.Length = 0 Then MsgBox("Please Enter Destination Number") : Exit Function
        If txtUserData = "" Then MsgBox("Please Enter UserData") : Exit Function
...
#End Region
    ...
    Private Sub startup()
        ...
        If TempPluginSettings.PhoneBookListUpdate = True Then 'mise à jour des phone book
            ...
            For Each PhoneBookList As String In PhoneBooKListArray 'Read each phone book usable and sav it
                ...
                Thread.Sleep(30000)
            Next
            debugFrm.debugTextBox.AppendText("Update phone books is done ..." & vbCrLf & vbCrLf)
        Else
    ...
    Private Sub HFPDone()
        ...
        'Btsdk_UnregisterHFPService(hfagService)
...
#Region "PBAP"
    Public Sub Phone_WriteContact()
        ...
        ATExecute("AT+CPBW=,""0102030405"",129,""Test""" & vbCrLf, 500)
    End Sub
    Public Sub PBAPRead()
...
#Region "PBAP tests"
    'see example here
    'http://www.vbforums.com/showthread.php?798767-Correct-Snytax-for-this-API&p=4902147&viewfull=1#post4902147
    'Public Sub PBAPParser_Test()

    '    Dim structPBAP As New BtSdkPBAPSvrCBStru
    '    With structPBAP
    '        .cardparser_rtns = New BtSdkPBAPParserRoutinesStru
    '        .findfile_rtns = New BtSdkPBAPFindFileRoutinesStru

    '        '.fileio_rtns = New BtSdkPBAPFileIORoutinesStru
    '        '.dirctrl_rtns = New BtSdkPBAPDirCtrlRoutinesStru
    '        '.get_new_missedcalls = Btsdk_PBAP_GetMissedCalls_Func
    '    End With

    '    structPBAP.cardparser_rtns.parse_open = AddressOf my_Btsdk_vCardParser_Open_Func
    '    structPBAP.findfile_rtns.find_first = AddressOf my_Btsdk_FindFirstFile_Func

    '    MessageBox.Show(Marshal.SizeOf(structPBAP).ToString)    '16 -> looks good

    '    Dim p As IntPtr = Marshal.AllocHGlobal(Marshal.SizeOf(structPBAP))
    '    Marshal.StructureToPtr(structPBAP, p, False)
    '    MessageBox.Show(Marshal.ReadInt32(p.ToInt32 + 0).ToString)  '1st pointer =set
    '    MessageBox.Show(Marshal.ReadInt32(p.ToInt32 + 4).ToString)  '2nd pointer =0
    '    MessageBox.Show(Marshal.ReadInt32(p.ToInt32 + 8).ToString)  '3rd pointer =set
    '    'Dim text As String = ""
    '    'For n = 0 To Marshal.SizeOf(structPBAP)
    '    '    For i = 0 To 8
    '    '        text &= Marshal.ReadInt32(p.ToInt32 + i).ToString & ","
    '    '    Next
    '    '    text &= vbCrLf
    ...
    'Public Function NumberOfMissedCall() As Integer
    '    Btsdk_PBAP_GetMissedCalls_Func(MainPath & "test.txt")
    'End Function




    Public Sub TestConnectPBAPSvc()
        Try
            'connectAG_PABP()
            'connPBAPService()
            RegisterPbapService()
            ...
            TestGetPhoneBook()
        Catch ex As Exception
            MessageBox.Show("PBAP error init -> " & ex.Message)
        End Try
    ...
    '	None.
    '---------------------------------------------------------------------------*/
    Private Sub GetRmtPSEAttribute(conn_hdl As UInt32, pattr As BtSdkRmtPSESvcAttrStru)
        Dim conn_prop As New BtSdkConnectionPropertyStru
        pattr.repositories = BTSDK_PBAP_REPO_LOCAL
        If Btsdk_GetConnectionProperty(conn_hdl, conn_prop) = BTSDK_OK Then
            Dim svc_attr As New BtSdkRemoteServiceAttrStru
            Dim psvc_attr As IntPtr = Marshal.AllocHGlobal(Marshal.SizeOf(GetType(BtSdkRemoteServiceAttrStru)))
            svc_attr.mask = BTSDK_RSAM_EXTATTRIBUTES

            If Btsdk_GetRemoteServiceAttributes(conn_prop.service_handle, Marshal.PtrToStructure(psvc_attr, GetType(BtSdkRemoteServiceAttrStru))) = BTSDK_OK Then
                'memcpy(pattr, svc_attr.ext_attributes, sizeof(BtSdkRmtPSESvcAttrStru));
                CopyMemory(pattr, svc_attr.ext_attributes, Marshal.SizeOf(GetType(BtSdkRmtPSESvcAttrStru)))
                Marshal.FreeHGlobal(svc_attr.ext_attributes)
    ...
    Private Shared Sub CopyMemory(ByVal dest As BtSdkRmtPSESvcAttrStru, ByVal src As IntPtr, ByVal count As Integer)
    End Sub


    Dim s_currPbapConnHdl As UInt32 = BTSDK_INVALID_HANDLE
    'see example here
    Dim g_root_pse As String
    'http://www.vbforums.com/showthread.php?798767-Correct-Snytax-for-this-API&p=4902147&viewfull=1#post4902147
    Private Sub RegisterPbapService()
        Try

            Dim svc_hdl As UInt32 = BTSDK_INVALID_HANDLE
            Dim svr_info As New BtSdkLocalPSEServerAttrStru
            Dim st_pbapsvr_cb As New BtSdkPBAPSvrCBStru

            svr_info.size = Marshal.SizeOf(GetType(BtSdkLocalPSEServerAttrStru))
            'MessageBox.Show(svr_info.size.ToString)
            g_root_pse = "C:\tmp\PSE"
            'MessageBox.Show(svr_info.root_dir.ToString)
            svr_info.root_dir = g_root_pse
            svr_info.path_delimiter = "\"
            svr_info.repositories = BTSDK_PBAP_REPO_LOCAL


            st_pbapsvr_cb.cardparser_rtns = New BtSdkPBAPParserRoutinesStru
            st_pbapsvr_cb.findfile_rtns = New BtSdkPBAPFindFileRoutinesStru
            st_pbapsvr_cb.fileio_rtns = New BtSdkPBAPFileIORoutinesStru

            '*those functions come from versit project.*
            st_pbapsvr_cb.cardparser_rtns.parse_open = New Btsdk_vCardParser_Open_Func(AddressOf vCardParse)
            st_pbapsvr_cb.cardparser_rtns.get_prop = New Btsdk_vCardParser_GetProperty_Func(AddressOf GetProperty)
            st_pbapsvr_cb.cardparser_rtns.parse_close = New Btsdk_vCardParser_Close_Func(AddressOf vCardClose)
            st_pbapsvr_cb.cardparser_rtns.get_prop = New Btsdk_vCardParser_GetProperty_Func(AddressOf GetProperty)
            st_pbapsvr_cb.cardparser_rtns.parse_free = New Btsdk_vCardParser_FreeProperty_Func(AddressOf FreevCardBuff)
            st_pbapsvr_cb.cardparser_rtns.parse_findfirst = New Btsdk_vCardParser_FindFirstProperty_Func(AddressOf FindFirstProperty)
            st_pbapsvr_cb.cardparser_rtns.parse_findnext = New Btsdk_vCardParser_FindNextProperty_Func(AddressOf FindNextProperty)
            st_pbapsvr_cb.cardparser_rtns.parse_findclose = New Btsdk_vCardParser_FindPropertyClose_Func(AddressOf EndFindProperty)

            '*those functions are usually platform dependent.*
            st_pbapsvr_cb.findfile_rtns.find_first = New Btsdk_FindFirstFile_Func(AddressOf APP_FindFirstFile)
            st_pbapsvr_cb.findfile_rtns.find_next = New Btsdk_FindNextFile_Func(AddressOf APP_FindNextFile)
            st_pbapsvr_cb.findfile_rtns.find_close = New Btsdk_FindFileClose_Func(AddressOf APP_FindFileClose)

            st_pbapsvr_cb.fileio_rtns.close_file = New Btsdk_CloseFile_Func(AddressOf APP_CloseFile)
            st_pbapsvr_cb.fileio_rtns.create_file = New Btsdk_CreateFile_Func(AddressOf APP_CreateFile)
            st_pbapsvr_cb.fileio_rtns.get_file_size = New Btsdk_GetFileSize_Func(AddressOf APP_GetFilesize)
            st_pbapsvr_cb.fileio_rtns.open_file = New Btsdk_OpenFile_Func(AddressOf APP_OpenFile)
            st_pbapsvr_cb.fileio_rtns.read_file = New Btsdk_ReadFile_Func(AddressOf APP_ReadFile)
            st_pbapsvr_cb.fileio_rtns.rewind_file = New Btsdk_RewindFile_Func(AddressOf APP_Rewind)
            st_pbapsvr_cb.fileio_rtns.write_file = New Btsdk_WriteFile_Func(AddressOf APP_WriteFile)

            st_pbapsvr_cb.dirctrl_rtns.change_dir = New Btsdk_ChangDir_Func(AddressOf APP_ChangeDir)
            st_pbapsvr_cb.dirctrl_rtns.create_dir = New Btsdk_CreateDir_Func(AddressOf APP_CreateDir)

            st_pbapsvr_cb.get_new_missedcalls = New Btsdk_PBAP_GetMissedCalls_Func(AddressOf APP_GetNewMissedCalls)


            ''MessageBox.Show(Marshal.SizeOf(st_pbapsvr_cb).ToString)    '16 -> looks good

            ''Dim p As IntPtr = Marshal.AllocHGlobal(Marshal.SizeOf(st_pbapsvr_cb))
            ''Marshal.StructureToPtr(st_pbapsvr_cb, p, False)
            ''MessageBox.Show(Marshal.ReadInt32(p.ToInt32 + 0).ToString)  '1st pointer =set
            ''MessageBox.Show(Marshal.ReadInt32(p.ToInt32 + 4).ToString)  '2nd pointer =0
            ''MessageBox.Show(Marshal.ReadInt32(p.ToInt32 + 8).ToString)  '3rd pointer =set
            ''Marshal.FreeHGlobal(p)
            ...
            If (svc_hdl <> BTSDK_INVALID_HANDLE) Then
                MessageBox.Show("Register pbap service successfully!")
                PBAPServiceIsReady = True
            Else
                MessageBox.Show("Register pbap service Failed!")
                PBAPServiceIsReady = False
            End If
        Catch ex As Exception
            MessageBox.Show("PBAP issue > " & ex.Message)
        End Try
    End Sub

    Private Sub PSERegisterPbapService()
        Try
            Dim svc_hdl As UInt32 = BTSDK_INVALID_HANDLE
            Dim svr_info As New BtSdkLocalPSEServerAttrStru
            Dim st_pbapsvr_cb As New BtSdkPBAPSvrCBStru
            ...
            svc_hdl = Btsdk_RegisterPBAPService("Phone Book Access Server", svr_info, st_pbapsvr_cb)
            If (svc_hdl <> BTSDK_INVALID_HANDLE) Then
                MessageBox.Show("Register PSEpbap service successfully!")
                PBAPServiceIsReady = True
            Else
                MessageBox.Show("Register PSEpbap service Failed!")
                PBAPServiceIsReady = False
            End If
        Catch ex As Exception
            MessageBox.Show("PBAP issue > " & ex.Message)
    ...
    Public Function vCardParse(file_hdl As Int32) As Byte
    End Function
    Public Function GetProperty(ByRef v_obj As Byte, ByRef prop As Byte, ByRef len As Int32) As Byte
    End Function
    Public Function vCardClose(ByRef v_obj As Byte) As Byte
    End Function
    Public Function FreevCardBuff(ByRef buf As Byte) As Byte
    End Function
    Public Function FindFirstProperty(ByRef v_obj As Byte, ByRef prop As Byte, ByRef len As Int32) As Byte
    End Function
    Public Function FindNextProperty(file_hdl As Int32, ByRef len As Int32) As Byte
    End Function
    Public Function EndFindProperty(file_hdl As Int32) As Byte
    End Function


    Public Function APP_FindFirstFile(file_hdl As Int32) As Byte
    End Function
    Public Function APP_FindNextFile(find_hdl As Int32, file_name As String) As Int32
    End Function
    Public Function APP_FindFileClose(file_hdl As Int32) As Byte
    End Function

    Public Function APP_CloseFile(file_hdl As Int32) As Byte
    End Function
    Public Function APP_CreateFile(find_hdl As Int32, file_name As String) As Int32
    End Function
    Public Function APP_GetFilesize(file_hdl As Int32) As Byte
    End Function
    Public Function APP_OpenFile(file_hdl As Int32) As Byte
    End Function
    Public Function APP_ReadFile(file_hdl As UInt32, buf As Byte, len As UInt32) As Int32
    End Function
    Public Function APP_Rewind(file_hdl As UInt32, offset As UInt32) As Int32
    End Function
    Public Function APP_WriteFile(file_hdl As UInt32, buf As Byte, bytes_to_write As UInt32) As Int32
    End Function
    Public Function APP_ChangeDir(path As Byte) As Int32
    End Function
    Public Function APP_CreateDir(path As Byte) As Int32
    End Function
    Public Function APP_GetNewMissedCalls(path As Byte) As Int32
    End Function

    'Public Function my_Btsdk_FindFirstFile_Func(path As String) As Int32
    '    'do something
    'End Function
    ...
    '	None. 
    '---------------------------------------------------------------------------*/
    Private Sub TestGetPhoneBook()
        Try

            Dim hFile As UInt32
            Dim pb As String = ""
            'You can pass this pointer to unmanaged functions
            Dim ptr As IntPtr
            Dim param As New BtSdkPBAPParamStru
            Dim strPath As String = ""
            Dim result As UInt32 = BTSDK_OK
            Dim pse_attr As New BtSdkRmtPSESvcAttrStru
            Dim hconn As UInt32 = connHandle 'pbapService 's_currPbapConnHdl
            Dim ch As String = "1"
            Dim fileio_rtns As New BtSdkPBAPFileIORoutinesStru

            'memset(&param, 0, sizeof(BtSdkPBAPParamStru));
            'see http://www.vbforums.com/showthread.php?678389-VC-to-VB-NET-Code-Conversion
            'Allocate unmanaged memory to hold structure data
            ptr = Marshal.AllocHGlobal(Marshal.SizeOf(param))
            'Copy structure's data to the allocated memory
            ...
            If hconn = BTSDK_INVALID_HANDLE Then
                MessageBox.Show("No PBAP connection selected!")
                Return
            Else
                MessageBox.Show("Test Get PhoneBook Ready !")
            End If


            fileio_rtns.write_file = AddressOf APP_WriteFile
            Btsdk_PBAPRegisterFileIORoutines(hconn, fileio_rtns)
            Btsdk_PBAPRegisterStatusCallback(hconn, AddressOf PbapAppStatusCB)

            Try
                param.mask = BTSDK_PBAP_PM_MAXCOUNT
                '6.2.3.74 of SDK 2.1.3
                param.max_count = 0 '&HFFFF 'Convert.ToUInt16("0xFFFF") 'Si vous voulez juste pour obtenir le nombre , ce paramètre est réglé sur 0
                '0x0000: the PCE would like to know the number of the vCard object in the phone book. 
                'In this case, the other parameters of this structure will be ignored and 
                'the server returns the number of the vCard object by pb_size.
                'MessageBox.Show("Nombre of vCard in phone is : " & param.pb_size)
                '0xFFFF: the PCE of the vCard does not limit the number of objects.
                'This parameter is only applicable to the request of PullPhoneBook and PullvCardListing.
                'If BTSDK_PBAP_PM_MAXCOUNT is not set in mask, max count is processed as 0xFFFF as default.

                'param.list_offset = BTSDK_PBAP_PM_LISTOFFSET
                'param.order = BTSDK_PBAP_PM_ORDER
                'If BTSDK_PBAP_PM_ORDER is not set in mask, vCard objects are arranged according to the storing number.
                'param.format = BTSDK_PBAP_FMT_VCARD21 'Specifies the format of vCard returned from server. (defaut is 2.1)
                'param.format = BTSDK_PBAP_FMT_VCARD30
                MessageBox.Show("Param OK")
            Catch ex As Exception
                MessageBox.Show("Param error : " & ex.Message)
            End Try
            ...
            Try
                ...
                MessageBox.Show("Repository is: " & pse_attr.repositories.ToString)
                If pse_attr.repositories = BTSDK_PBAP_REPO_LOCAL Then
                    pb = "telecom/" : MessageBox.Show("LOCAL")
                ElseIf pse_attr.repositories = BTSDK_PBAP_REPO_SIM Then
                    pb = "SIM1/telecom/" : MessageBox.Show("SIM")
                End If
                MessageBox.Show("Selection REPO OK is: " & pb)
            Catch ex As Exception
                MessageBox.Show("CallBack error is :" & ex.Message)
            End Try

            Select Case ch
                Case "1" : pb &= "pb.vcf"  'Main Phone book
                Case "2" : pb &= "ich.vcf" 'Incoming Calls History
                Case "3" : pb &= "och.vcf" 'Outgoing Calls History
                Case "4" : pb &= "mch.vcf" 'Missed Calls History
                Case "5" : pb &= "cch.vcf" 'Combined Calls History
            End Select

            If ch <> "1" Then
                'MessageBox.Show("Filter type -> " & param.filter(0))
                '   'function sets the attribute masks table
                '   Btsdk_PBAPFilterComposer(param.filter(0), BTSDK_PBAP_FILTER_X_IRMC_CALL_DATETIME) ' return the error 0x304

                'function sets the attribute masks table (see all flag possible 6.3.9.1.28 of SDK 2.13
                param.filter = Convert.ToByte(BTSDK_PBAP_FILTER_X_IRMC_CALL_DATETIME)
            End If
            ...
            hFile = APP_CreateFile("c:\test.lst")
            result = Btsdk_PBAPPullPhoneBook(hconn, pb, param, hFile)


            If result = BTSDK_OK Then
                MessageBox.Show("The phone book retrieved is saved to " & strPath)
            Else
                MessageBox.Show("Error is -> " & SDKErrorToString(Hex(result)))
            End If

            'free(strPath);
            APP_CloseFile(hFile)
        Catch ex As Exception
            MessageBox.Show("TestGetPhoneBook error -> '" & ex.Message & "'")
            APP_CloseFile("c:\test.lst")
        End Try
    End Sub
    ...
    '	None. 
    '---------------------------------------------------------------------------*/
    Private Sub PbapAppStatusCB(first As String, last As String, filename As String, filesize As UInt32, cursize As UInt32)
        Dim s_cur_size As UInt32 = 0
        Dim s_total_size As UInt32 = 0

        If last = "" Then
            If first <> "" Then
                s_total_size = filesize
                'printf("\nstart transferring %s:\n", filename);
                'printf("%10d byte / %d transferred", s_cur_size, s_cur_size);
                MessageBox.Show("Filename: " & filename & vbCrLf & "CurSize: " & s_cur_size)
            Else
                s_cur_size += cursize
                'printf("\r%10d bytes", s_cur_size);
                MessageBox.Show("CurSize: " & s_cur_size)
            End If
        Else
            s_cur_size += cursize
            'printf("\r%10d byte / %d transferred\n", s_cur_size, s_total_size);
            MessageBox.Show("CurSize: " & s_cur_size & vbCrLf & "TotalSize: " & s_total_size)
            s_cur_size = 0
            s_total_size = 0
        End If
    ...
    '	BTSDKHANDLE 
    '---------------------------------------------------------------------------*/
    Private Function APP_CreateFile(file_name As String) As UInt32
        If file_name <> "" Then
            Dim objStreamWriter As StreamWriter
            objStreamWriter = New StreamWriter(file_name, True, Encoding.Unicode)
            Return 1
        Else
            Return 0
        End If
    ...
    '	BTUINT32 
    '---------------------------------------------------------------------------*/
    Private Function APP_WriteFile(file_name As String) As UInt32
        If file_name <> "" Then
            Dim objStreamWriter As StreamWriter
            objStreamWriter = New StreamWriter(file_name, True, Encoding.Unicode)
    ...
    '	void 
    '---------------------------------------------------------------------------*/
    Private Sub APP_CloseFile(file_hdl As StreamWriter)
        file_hdl.Close()
