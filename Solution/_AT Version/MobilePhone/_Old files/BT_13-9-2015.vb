...
#End Region
    ...
    Public Sub requestPhoneInfo()
        ATExecute("ATE1" & vbCrLf, 200) 'Echo ON
        'ATExecute("AT+CGMM" & vbCrlf, 2000) 'phone manufacturer
        'ATExecute("AT+CGMI" & vbCrlf, 2000) 'phone model
        Thread.Sleep(100)
        ATExecute("AT+CGMR" & vbCrLf, 200) 'Revision number
        Thread.Sleep(100)
        ATExecute("AT+CIMI" & vbCrLf, 200) 'ISMI number
        Thread.Sleep(100)
        ATExecute("AT+CGSN" & vbCrLf, 200) 'IMEI number
        Thread.Sleep(100)
        ATExecute("AT+CNUM" & vbCrLf, 200) 'subscriber number
        'Btsdk_HFAP_GetSubscriberNumber(connHandle) 'subscriber number
        Thread.Sleep(100)
        ATExecute("AT+CSCA?" & vbCrLf, 200) 'SMS service center number
        Thread.Sleep(100)
        ATExecute("AT+CCLK?" & vbCrLf, 200) 'Phone Time and Date
        Thread.Sleep(100)
